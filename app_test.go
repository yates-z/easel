...
import (
	...
	grpc "github.com/yates-z/easel/transport/grpc/server"
	http "github.com/yates-z/easel/transport/http/server"
	"net/url"
	"reflect"
	"sync"
	"testing"
...
func TestApp_Endpoint(t *testing.T) {
	...
	if !reflect.DeepEqual(o.Endpoints(), v) {
		t.Errorf("Endpoint() = %v, want %v", o.Endpoints(), v)
	}
}
...
func TestApp_Context(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			ctx := context.WithValue(context.Background(), APP_KEY, a)
			...
			if got, ok := FromContext(ctx); ok {
				...
				if !reflect.DeepEqual(got.Endpoints(), tt.want.endpoint) {
					t.Errorf("Endpoint() = %v, want %v", got.Endpoints(), tt.want.endpoint)
				}
