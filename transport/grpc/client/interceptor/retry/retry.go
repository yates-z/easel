...
import (
	...
	"github.com/yates-z/easel/transport/grpc/utils/metadata"
...
func UnaryClientInterceptor(max uint, interval time.Duration, opts ...Option) grpc.UnaryClientInterceptor {
	callOpts := &options{
		max:           max,
		includeHeader: true,
		backoffFunc:   BackoffLinearWithJitter(interval /*jitter*/, 0.10),
		onRetryCallback: func(ctx context.Context, attempt uint, err error) {
			logAndTrace(ctx, "grpc_retry attempt: %d, backoff for %v", attempt, err)
		},
	...
	return func(parentCtx context.Context, method string, req, reply any, cc *grpc.ClientConn, invoker grpc.UnaryInvoker, opts ...grpc.CallOption) error {
		...
		var lastErr error
		for attempt := range callOpts.max {
			...
			if isContextError(lastErr) {
				if parentCtx.Err() != nil {
					logAndTrace(parentCtx, "grpc_retry attempt: %d, parent context error: %v", attempt, parentCtx.Err())
					...
					logAndTrace(parentCtx, "grpc_retry attempt: %d, context error from retry call", attempt)
					continue
				}
			}
...
func StreamClientInterceptor(max uint, interval time.Duration, opts ...Option) grpc.StreamClientInterceptor {
	...
	return func(parentCtx context.Context, desc *grpc.StreamDesc, cc *grpc.ClientConn, method string, streamer grpc.Streamer, opts ...grpc.CallOption) (grpc.ClientStream, error) {
		...
		for attempt := uint(0); attempt < callOpts.max; attempt++ {
			...
			newStreamer, lastErr = streamer(parentCtx, desc, cc, method, opts...)
			if lastErr == nil {
...
func (s *serverStreamingRetryingStream) receiveMsgAndIndicateRetry(m any) (bool, error) {
	...
	if isContextError(err) {
		if s.parentCtx.Err() != nil {
			logAndTrace(s.parentCtx, "grpc_retry parent context error: %v", s.parentCtx.Err())
			return false, err
		} else if s.callOpts.perCallTimeout != 0 {
			// We have set a perCallTimeout in the retry middleware, which would result in a context error if
			// the deadline was exceeded, in which case try again.
			logAndTrace(s.parentCtx, "grpc_retry context error from retry call")
			return true, err
		}
	}
...
func (s *serverStreamingRetryingStream) reestablishStreamAndResendBuffer(callCtx context.Context) (grpc.ClientStream, error) {
	...
	if err != nil {
		logAndTrace(callCtx, "grpc_retry failed redialing new stream: %v", err)
		return nil, err
	}
	for _, msg := range bufferedSends {
		if err := newStream.SendMsg(msg); err != nil {
			logAndTrace(callCtx, "grpc_retry failed resending message: %v", err)
			return nil, err
		}
	}
	if err := newStream.CloseSend(); err != nil {
		logAndTrace(callCtx, "grpc_retry failed CloseSend on new stream %v", err)
		return nil, err
	}
...
func waitRetryBackoff(attempt uint, parentCtx context.Context, callOpts *options) error {
	...
	if waitTime > 0 {
		logAndTrace(parentCtx, "grpc_retry attempt: %d, backoff for %v", attempt, waitTime)
...
func logAndTrace(ctx context.Context, format string, a ...any) {
	logger.Context(ctx).Infof(format, a...)
