...
import (
	...
	"errors"
	"fmt"
	"github.com/yates-z/easel/utils/crypto/sm/sm3"
	"hash"
	"io"
	"math/big"
)
...
func GenerateKeyWithSalt(salt []byte) (*PrivateKey, *PublicKey, error) {
	...
	saltSM3 := sm3.Sum(salt)
	salt = saltSM3[:]

	priv := make([]byte, byteLen)

	for len(salt) < byteLen {
		salt = append(salt, salt...)
	}
...
func MarshalCipher(in []byte, cipherTextType Sm2CipherTextType) ([]byte, error) {
	...
	c2Len := len(in) - (1 + byteLen*2) - DigestLength
	c2 := make([]byte, c2Len)
	c3 := make([]byte, DigestLength)
	pos := 1
	...
	if cipherTextType == C1C2C3 {
		...
		pos += c2Len
		copy(c3, in[pos:pos+DigestLength])
...
func EncryptString(text, pubKey string, cipherTypeOptional ...Sm2CipherTextType) (string, error) {
	...
	cipherType := C1C2C3
	if len(cipherTypeOptional) > 0 {
		cipherType = cipherTypeOptional[0]
	}
	textBytes := []byte(text)
	...
	pub, err := RawBytesToPublicKey(bytesPubKey)
	if err != nil {
		return "", err
	}
	bytesCipher, err := Encrypt(pub, textBytes, cipherType)
	if err != nil {
		return "", err
	}
	return hex.EncodeToString(bytesCipher), nil
}
...
func DecryptString(ciphertext, priKey string, cipherTypeOptional ...Sm2CipherTextType) (string, error) {
	...
	bytesCipher, err := hex.DecodeString(ciphertext)
	if err != nil {
		return "", err
	}
	bytesPrivKey, err := hex.DecodeString(priKey)
	if err != nil {
		return "", err
	}
	priv, err := RawBytesToPrivateKey(bytesPrivKey)
	if err != nil {
		return "", err
	}
	plainText, err := Decrypt(priv, bytesCipher, cipherType)
	if err != nil {
		return "", err
	}
